package com.pasterdream.pasterdreammod.api.item.manager;
...
public class ItemManager {
     ...
     * 批量注册简易材料物品
     * <p>
     * 遍历传入的 {@link ItemSpec} 列表，使用 {@link SimpleItemBuilder} 逐一构建并注册到 {@link ItemAPI#REGISTRY}，
    ...
    public List<DeferredItem<Item>> batchRegisterSimple(List<ItemSpec> specs) {
        ...
        for (ItemSpec spec : specs) {
            try {
                SimpleItemBuilder builder = new SimpleItemBuilder(ItemAPI.REGISTRY, spec.registryName())
     ...
     * 批量注册食物物品
     * <p>
     * 遍历传入的名称到食物规格映射，使用 {@link FoodItemBuilder} 逐一构建并注册到 {@link ItemAPI#REGISTRY}，
    ...
    public List<DeferredItem<Item>> batchRegisterFood(Map<String, FoodSpec> specs) {
        ...
        for (Map.Entry<String, FoodSpec> entry : specs.entrySet()) {
            ...
            try {
                FoodItemBuilder builder = new FoodItemBuilder(ItemAPI.REGISTRY, name)
