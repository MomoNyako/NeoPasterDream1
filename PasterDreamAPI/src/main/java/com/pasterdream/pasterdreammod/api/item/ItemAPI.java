 ...
 * - Facade模式：ItemAPI 提供统一入口，屏蔽内部实现细节
 ...
 * ===========================================================================
 */
package com.pasterdream.pasterdreammod.api.item;

import com.pasterdream.pasterdreammod.api.PasterDreamAPI;
import com.pasterdream.pasterdreammod.api.item.builder.CurioItemBuilder;
import com.pasterdream.pasterdreammod.api.item.builder.FoodItemBuilder;
import com.pasterdream.pasterdreammod.api.item.builder.SimpleItemBuilder;
import com.pasterdream.pasterdreammod.api.item.builder.ToolItemBuilder;
import com.pasterdream.pasterdreammod.api.item.manager.ItemManager;
import com.pasterdream.pasterdreammod.api.item.model.FoodSpec;
import com.pasterdream.pasterdreammod.api.item.model.ItemSpec;
 ...
 * 内部封装了 {@link DeferredRegister.Items}、{@link ItemManager} 等核心组件，
 * 外部调用者无需关心注册细节。
 * <p>
 * <b>设计架构：</b>
 * <pre>
...
public final class ItemAPI {
     ...
     * <b>使用示例：</b>
     * <pre>{@code
     * String lang = ItemAPI.generateLangJson("pasterdream", Map.of(
     *     "item.pasterdream.titanium_ingot", "钛锭",
     *     "item.pasterdream.apple_juice", "苹果汁"
     * ));
    ...
    /**
     * 获取 {@link ItemManager} 管理器实例。
     * <p>
     * 当需要执行更高级的迁移管理操作（如自定义分类统计、导出迁移清单等）
     * 时，可通过此方法获取管理器实例进行直接操作。
     *
     * @return {@link ItemManager} 单例实例
     */
     ...
     * 重置所有静态追踪状态，供测试使用。
     * <p>
     * 清空 {@link ItemManager} 中维护的已移植、待移植、警告等追踪数据，
     * 使每次测试都在干净的追踪状态下运行。
     * 注意：此方法不会取消 DeferredRegister 中的已注册物品，仅清除 API 层面的追踪缓存。
     */
     ...
     */
    private ItemAPI() {
        throw new UnsupportedOperationException("ItemAPI 是纯静态门面类，不可实例化");
    }
}
