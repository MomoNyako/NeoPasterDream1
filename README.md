...
|   优先级  | 问题                                                              | 建议                                                                          |       修复状态       |
| :----: | --------------------------------------------------------------- | --------------------------------------------------------------------------- | :--------------: |
| **P0** | 粒子 Provider 缺失                                                  | 为 SHADOW\_STONE、SPORE、FOX\_FIRE\_0/1 新建粒子类并在 `ClientSetup` 注册 Provider      | 未修复（由LOVE\_U来完成） |
| **P0** | `DimensionApiDemo` 引用客户端类                                       | 将 `dimension/example/` 整体迁移到 `src/test/java` 或独立工具模块，避免 dedicated server 加载 | 未修复（由LOVE\_U来完成） |
...
| **P1** | `CurioBuilder` 使用已弃用 `ICurioItem` API                           | 迁移到 `getAttributeModifiers(SlotContext, ItemStack)`，消除编译警告                  |        未修复       |
| **P1** | `CurioAPI.registerClientRenderers()` 无 side 校验                  | 方法入口增加 `FMLEnvironment.dist == Dist.CLIENT` 保护                              |        未修复       |
...
| **P1** | 缺少统一注册入口                                                        | 新增 `PasterDreamAPI.registerAll(modEventBus)`                                | 修复中（由LOVE\_U来完成） |
