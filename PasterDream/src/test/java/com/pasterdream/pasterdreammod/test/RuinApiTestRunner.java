...
public class RuinApiTestRunner {
    ...
    private static void testRuinQueryMethods() {
        try {
            ...
            // 测试 getRuin 对未注册结构返回空 Optional
            java.util.Optional<RuinResult> resultOpt = RuinAPI.getRuin("non_existent_ruin");
            if (resultOpt.isEmpty()) {
                passed++;
                printPass("RuinAPI.getRuin() 正确返回空 Optional（未注册的结构）");
            } else {
                failed++;
                printFail("RuinAPI.getRuin() 预期空 Optional 但返回了结果");
            }
        } catch (Exception e) {
            failed++;
