...
public class ParticleApiTestRunner {
    ...
    private static void testParticleCaching() {
        ...
        try {
            java.util.Optional<ParticleResult> cached = ParticleAPI.getParticle(particleName);
            if (cached.isEmpty()) {
                passed++;
                printPass("缓存查询正确返回空 Optional（尚未注册）: " + particleName);
            } else {
                failed++;
                printFail("缓存查询预期返回空 Optional，但得到了结果: " + cached.get());
            }
        } catch (Exception e) {
            failed++;
