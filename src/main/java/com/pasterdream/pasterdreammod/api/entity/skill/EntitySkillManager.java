...
public class EntitySkillManager {
    ...
    public EntitySkillManager(Mob entity) {
        this.entity = entity;
        PasterDreamMod.LOGGER.debug("[EntitySkillManager] 创建技能管理器: entity={}", entity);
    }
    ...
    public EntitySkillManager registerSkill(EntitySkill skill) {
        ...
        PasterDreamMod.LOGGER.info("[EntitySkillManager] 注册技能: {} → {} | 技能总数: {}", entity, skill.name(), skills.size());
        return this;
    }
    ...
    public EntitySkillManager registerSkills(EntitySkill... skillArray) {
        ...
        PasterDreamMod.LOGGER.info("[EntitySkillManager] 批量注册技能完成: {} 个技能已注册", skillArray.length);
        return this;
    }
    ...
    public boolean tryTriggerSkill(String skillName, @Nullable LivingEntity target) {
        // 检查是否有技能正在执行
        if (skillTimer > 0) {
            PasterDreamMod.LOGGER.debug("[EntitySkillManager] 技能 [{}] 触发失败: 有技能正在执行 [{}]", skillName,
        ...
        if (skill == null) {
            PasterDreamMod.LOGGER.warn("[EntitySkillManager] 技能 [{}] 未注册到实体 {}", skillName, entity);
            return false;
        }
        ...
        if (remainingCooldown > 0) {
            PasterDreamMod.LOGGER.debug("[EntitySkillManager] 技能 [{}] 冷却中: 剩余 {} tick", skillName, remainingCooldown);
            return false;
        }
    ...
    protected void startSkill(EntitySkill skill, @Nullable LivingEntity target) {
        PasterDreamMod.LOGGER.info("[EntitySkillManager] 🎯 技能触发: {} | 施法者={}, 目标={}", skill.name(), entity, target);
    ...
    protected void finishCurrentSkill() {
        if (currentSkill != null) {
            PasterDreamMod.LOGGER.info("[EntitySkillManager] ✅ 技能完成: {}", currentSkill.name());
        }
        skillTimer = 0;
        currentSkill = null;
        setAnimation("empty");
    ...
    protected void executeSkillEffects(EntitySkill skill) {
        PasterDreamMod.LOGGER.debug("[EntitySkillManager] 执行技能效果: {} | damage={}, range={}", skill.name(), skill.damage(), skill.range());
        ...
        PasterDreamMod.LOGGER.info("[EntitySkillManager] 💥 技能 [{}] 对 {} 个目标造成 {} 点伤害", skill.name(), targets.size(), skill.damage());
    }
    ...
    protected void spawnSkillParticles(ServerLevel level, EntitySkill skill) {
        ...
        if (particleType == null) {
            PasterDreamMod.LOGGER.warn("[EntitySkillManager] 粒子 [{}] 未找到，跳过粒子生成", skill.particleName());
            return;
        }
        ...
        PasterDreamMod.LOGGER.debug("[EntitySkillManager] ✨ 技能 [{}] 生成粒子: {} (x30)", skill.name(), skill.particleName());
    }
    ...
    protected void playSkillSound(EntitySkill skill) {
        ...
        PasterDreamMod.LOGGER.debug("[EntitySkillManager] 🔊 技能 [{}] 播放音效: {}", skill.name(), skill.soundId());
    }
    ...
    protected void setAnimation(String animation) {
        ...
        PasterDreamMod.LOGGER.debug("[EntitySkillManager] 🎬 设置动画: entity={}, animation={}", entity, animation);
    }
