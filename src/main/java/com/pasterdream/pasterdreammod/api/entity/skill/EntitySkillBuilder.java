...
public class EntitySkillBuilder {
    ...
    public EntitySkillBuilder(String name) {
        this.name = name;
        PasterDreamMod.LOGGER.debug("[EntitySkillBuilder] 创建技能构建器: {}", name);
    }
    ...
    public EntitySkillBuilder animationName(String animationName) {
        ...
        PasterDreamMod.LOGGER.debug("[EntitySkillBuilder] {} → animationName={}", name, animationName);
        return this;
    }
    ...
    public EntitySkillBuilder damage(float damage) {
        this.damage = damage;
        PasterDreamMod.LOGGER.debug("[EntitySkillBuilder] {} → damage={}", name, damage);
        return this;
    }
    ...
    public EntitySkillBuilder range(float range) {
        this.range = range;
        PasterDreamMod.LOGGER.debug("[EntitySkillBuilder] {} → range={}", name, range);
        return this;
    }
    ...
    public EntitySkillBuilder cooldownTicks(int cooldownTicks) {
        ...
        PasterDreamMod.LOGGER.debug("[EntitySkillBuilder] {} → cooldownTicks={} ({}秒)", name, cooldownTicks, cooldownTicks / 20.0);
        return this;
    }
    ...
    public EntitySkillBuilder particle(String particleName) {
        ...
        PasterDreamMod.LOGGER.debug("[EntitySkillBuilder] {} → particle={}", name, particleName);
        return this;
    }
    ...
    public EntitySkillBuilder sound(String soundId) {
        ...
        PasterDreamMod.LOGGER.debug("[EntitySkillBuilder] {} → sound={}", name, soundId);
        return this;
    }
    ...
    public EntitySkill build() {
        PasterDreamMod.LOGGER.info("[EntitySkillBuilder] ===== 开始构建技能: {} =====", name);
        validate();
        ...
        PasterDreamMod.LOGGER.info("[EntitySkillBuilder] ✅ 技能构建完成: {} | damage={}, range={}, cooldown={}tick, anim={}, particle={}, sound={}",
    ...
    private void validate() {
        if (animationName == null || animationName.isEmpty()) {
            PasterDreamMod.LOGGER.error("[EntitySkillBuilder] ❌ 验证失败: {} → animationName 未设置", name);
            throw new IllegalStateException("EntitySkillBuilder: 缺少动画名称（animationName），请调用 .animationName() 设置");
        }
        if (damage < 0) {
            PasterDreamMod.LOGGER.error("[EntitySkillBuilder] ❌ 验证失败: {} → damage 无效: {}", name, damage);
            throw new IllegalStateException("EntitySkillBuilder: 伤害值不能为负数");
        }
        if (range <= 0) {
            PasterDreamMod.LOGGER.error("[EntitySkillBuilder] ❌ 验证失败: {} → range 无效: {}", name, range);
            throw new IllegalStateException("EntitySkillBuilder: 作用范围必须大于 0");
        }
        if (cooldownTicks <= 0) {
            PasterDreamMod.LOGGER.error("[EntitySkillBuilder] ❌ 验证失败: {} → cooldownTicks 无效: {}", name, cooldownTicks);
            throw new IllegalStateException("EntitySkillBuilder: 冷却时间必须大于 0");
        }
        PasterDreamMod.LOGGER.debug("[EntitySkillBuilder] ✅ 验证通过: {}", name);
    }
}