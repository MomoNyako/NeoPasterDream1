...
public class EntityBuilder<T extends Entity> {
    ...
    public EntityBuilder(DeferredRegister<EntityType<?>> registry, String modId, String name) {
        ...
        this.modId = modId;
        this.name = name;
        PasterDreamMod.LOGGER.debug("[EntityBuilder] 创建实体构建器: modId={}, name={}", modId, name);
    }
    ...
    public EntityBuilder<T> category(MobCategory category) {
        ...
        PasterDreamMod.LOGGER.debug("[EntityBuilder] {} → category={}", name, category.getName());
        return this;
    }
    ...
    public EntityBuilder<T> size(float width, float height) {
        this.width = width;
        this.height = height;
        PasterDreamMod.LOGGER.debug("[EntityBuilder] {} → size={}x{}", name, width, height);
        return this;
    }
    ...
    public EntityBuilder<T> trackingRange(int trackingRange) {
        ...
        PasterDreamMod.LOGGER.debug("[EntityBuilder] {} → trackingRange={}", name, trackingRange);
        return this;
    }
    ...
    public EntityBuilder<T> updateInterval(int updateInterval) {
        ...
        PasterDreamMod.LOGGER.debug("[EntityBuilder] {} → updateInterval={}", name, updateInterval);
        return this;
    }
    ...
    public EntityBuilder<T> velocityUpdates(boolean velocityUpdates) {
        ...
        PasterDreamMod.LOGGER.debug("[EntityBuilder] {} → velocityUpdates={}", name, velocityUpdates);
        return this;
    }
    ...
    public <X extends Entity> EntityBuilder<X> entityClass(Class<X> entityClass) {
        ...
        PasterDreamMod.LOGGER.debug("[EntityBuilder] {} → entityClass={}", name, entityClass.getSimpleName());
    ...
    public EntityBuilder<T> attributes(Supplier<AttributeSupplier.Builder> attributesSupplier) {
        ...
        PasterDreamMod.LOGGER.debug("[EntityBuilder] {} → attributes=已配置（Builder 模式）", name);
        return this;
    }
    ...
    public EntityBuilder<T> attributesBuilt(Supplier<AttributeSupplier> attributesSupplier) {
        ...
        PasterDreamMod.LOGGER.debug("[EntityBuilder] {} → attributes=已配置（直接 Supplier）", name);
        return this;
    }
    ...
    public EntityBuilder<T> spawnEgg(int backgroundColor, int highlightColor) {
        ...
        PasterDreamMod.LOGGER.debug("[EntityBuilder] {} → spawnEgg=#{}/#{}",
    ...
    public EntityResult<T> build() {
        PasterDreamMod.LOGGER.info("[EntityBuilder] ===== 开始构建实体: {} =====", name);
        validate();

        PasterDreamMod.LOGGER.debug("[EntityBuilder] 构建 EntityType.Builder: category={}, size={}x{}, tracking={}, interval={}, velocity={}",
        ...
        PasterDreamMod.LOGGER.debug("[EntityBuilder] 注册实体类型: name={}, descriptionId={}", name, descriptionId);
        ...
        PasterDreamMod.LOGGER.debug("[EntityBuilder] 创建 EntityResult: name={}, holder={}, entityClass={}",
                name, holder, entityClass.getSimpleName());

        EntityAPI.cacheEntity(result);

        if (attributesSupplier != null) {
            PasterDreamMod.LOGGER.debug("[EntityBuilder] 缓存实体属性: {}", name);
            EntityAPI.cacheAttributes(name, attributesSupplier);
        } else {
            PasterDreamMod.LOGGER.debug("[EntityBuilder] 未配置属性，跳过属性缓存: {}", name);
        }

        if (spawnEggBackgroundColor != null && spawnEggHighlightColor != null) {
            PasterDreamMod.LOGGER.debug("[EntityBuilder] 缓存生成蛋颜色: {}", name);
    ...
    private void validate() {
        if (category == null) {
            PasterDreamMod.LOGGER.error("[EntityBuilder] ❌ 验证失败: {} → category 未设置", name);
            throw new IllegalStateException("EntityBuilder: 缺少实体分类（category），请调用 .category() 设置");
        }
        if (width <= 0 || height <= 0) {
            PasterDreamMod.LOGGER.error("[EntityBuilder] ❌ 验证失败: {} → size 未设置 (width={}, height={})", name, width, height);
            throw new IllegalStateException("EntityBuilder: 缺少碰撞箱尺寸（size），请调用 .size() 设置");
        }
        if (entityClass == null) {
            PasterDreamMod.LOGGER.error("[EntityBuilder] ❌ 验证失败: {} → entityClass 未设置", name);
            throw new IllegalStateException("EntityBuilder: 缺少实体类（entityClass），请调用 .entityClass() 设置");
        }
        PasterDreamMod.LOGGER.debug("[EntityBuilder] ✅ 验证通过: {}", name);
    }
    ...
    private T createEntity(EntityType<T> type, Level level) {
        try {
            ...
            PasterDreamMod.LOGGER.error("[EntityBuilder] ❌ 无法创建实体实例 [{}]: {}", name, e.getMessage(), e);
