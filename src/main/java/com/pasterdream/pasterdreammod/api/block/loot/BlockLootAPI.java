...
public final class BlockLootAPI {
    ...
    public static void selfDrop(String blockName) {
        ...
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== selfDrop() 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 方块名称: {}, 完整ID: {}", blockName, blockId);
        String json = LootTableGenerator.generateSelfDrop(blockId);
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 即将保存战利品表文件 → {}", blockName);
        saveToFile(blockName, json);
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ selfDrop() 完成: {}", blockName);
    }
    ...
    public static void oreDrop(String blockName, String dropItemId) {
        ...
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== oreDrop() 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 方块: {}, 掉落物品ID: {}, 完整ID: {}", blockName, dropItemId, fullId);
        String json = LootTableGenerator.generateOreDrop(fullId, blockName);
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 即将保存战利品表文件 → {}", blockName);
        saveToFile(blockName, json);
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ oreDrop() 完成: {} → {}", blockName, fullId);
    }
    ...
    public static void silkTouchDrop(String blockName) {
        String blockId = MOD_ID + ":" + blockName;
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== silkTouchDrop() 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 方块: {}, 完整ID: {}", blockName, blockId);
        String json = LootTableGenerator.generateSilkTouchDrop(blockId);
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 即将保存战利品表文件 → {}", blockName);
        saveToFile(blockName, json);
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ silkTouchDrop() 完成: {}", blockName);
    }
    ...
    public static void customDrop(String blockName, String dropItemId,
        ...
        String fullId = ensureNamespace(dropItemId);
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== customDrop() 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 方块: {}, 掉落物品: {}, silkTouchOnly={}, fortuneEnabled={}, explosionDecay={}",
        ...
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 即将保存战利品表文件 → {}", blockName);
        saveToFile(blockName, json);
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ customDrop() 完成: {}", blockName);
    }
    ...
    public static void multiDrop(String blockName, List<DropEntry> entries) {
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== multiDrop() 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 方块: {}, 条目数量: {}", blockName, entries.size());
        String json = LootTableGenerator.generateMultiDrop(entries, blockName);
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 即将保存战利品表文件 → {}", blockName);
        saveToFile(blockName, json);
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ multiDrop() 完成: {}", blockName);
    }
    ...
    public static void selfDropAll(String... blockNames) {
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== selfDropAll() 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 待生成总数: {} 个方块", blockNames.length);
        for (String name : blockNames) {
            PasterDreamMod.LOGGER.info("[BlockLootAPI]   → 生成 selfDrop: {}", name);
            selfDrop(name);
        }
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ selfDropAll() 完成, 共 {} 个方块", blockNames.length);
    }
    ...
    public static void oreDropAll(OreDropPair... drops) {
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== oreDropAll() 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 待生成总数: {} 个矿石", drops.length);
        for (OreDropPair pair : drops) {
            PasterDreamMod.LOGGER.info("[BlockLootAPI]   → 生成 oreDrop: {} → {}", pair.blockName(), pair.dropItemId());
            oreDrop(pair.blockName(), pair.dropItemId());
        }
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ oreDropAll() 完成, 共 {} 个矿石", drops.length);
    }
    ...
    public static void variantSetDropAll(String baseName) {
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== variantSetDropAll() 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 基础方块: {}, 变体: 默认9种", baseName);
        variantSetDropAll(baseName,
                "_stairs", "_slab", "_wall",
                "_fence", "_fencegate",
                "_door", "_trapdoor",
                "_pressure_plate", "_button");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ variantSetDropAll() 完成: {}", baseName);
    }
    ...
    public static void variantSetDropAll(String baseName, String... suffixes) {
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== variantSetDropAll(自定义后缀) 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 基础方块: {}, 后缀数量: {} >> {}", baseName, suffixes.length, String.join(", ", suffixes));
        for (String suffix : suffixes) {
            selfDrop(baseName + suffix);
        }
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ variantSetDropAll(自定义后缀) 完成: {}", baseName);
    }
    ...
    public static void batchDropSelf(String baseName, int... indices) {
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== batchDropSelf() 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 基础名称: {}, 编号数量: {}", baseName, indices.length);
        StringBuilder sb = new StringBuilder("编号: ");
        for (int idx : indices) {
            if (sb.length() > 4) sb.append(", ");
            sb.append(idx);
        }
        PasterDreamMod.LOGGER.info("[BlockLootAPI] {}", sb);
        for (int index : indices) {
            selfDrop(baseName + "_" + index);
        }
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ batchDropSelf() 完成: {}", baseName);
    }
    ...
    public static void batchDropSelfRange(String baseName, int start, int end, int... excludeIndices) {
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ===== batchDropSelfRange() 被调用 =====");
        PasterDreamMod.LOGGER.info("[BlockLootAPI] 基础名称: {}, 范围: [{}, {}], 排除数量: {}", baseName, start, end, excludeIndices.length);
        ...
        PasterDreamMod.LOGGER.info("[BlockLootAPI] ✅ batchDropSelfRange() 完成: {}, 实际生成: {} 个", baseName, count);
    }
    ...
    private static void saveToFile(String blockName, String json) {
        PasterDreamMod.LOGGER.info("[BlockLootAPI] → 调用 LootTableGenerator.saveLootTableToFile() 方块={}", blockName);
        try {
            LootTableGenerator.saveLootTableToFile(json, MOD_ID, blockName, BASE_PATH);
        } catch (IOException e) {
            PasterDreamMod.LOGGER.error("[BlockLootAPI] ❌ 无法保存战利品表文件 [{}]: {}", blockName, e.getMessage(), e);
